...
export interface Building {
  ...
  // --- ESG addon -----------------------------------------------------------
  /**
   * Continuous electrical draw in kW, before cooling overhead. The one physical
   * number in the addon: the power bill, the carbon term and the water draw all
   * come off it.
   *
   * Only nodes that own or occupy hardware carry one. A rented rate limit
   * carries a small attributed share and nothing more, which is the honest
   * asymmetry — renting does not make the power go away, it moves it onto
   * somebody else's meter.
   */
  powerKw?: number;
  /**
   * 0-10 site pressure. Only nodes that occupy real estate. Never negative:
   * land, unlike a subscription, does not come back.
   */
  landUse?: number;
  /**
   * 0-10 into the Social score: how much invisible human work this node leans
   * on. NEGATIVE on the two nodes that pay for it properly.
   */
  laborLoad?: number;
  /**
   * 0-10 into the Governance score: undisclosed provenance and restricted-origin
   * supply. NEGATIVE on the ledger and the audit.
   */
  provenanceRisk?: number;
  /**
   * Fraction of the whole estate's load covered by a contracted clean supply.
   * Estate-wide, not per node — a PPA is signed for a company, not a rack — so
   * the engine takes the best one running.
   */
...
export const BUILDINGS: Building[] = [
  ...
  B({ id: 'support_desk',  name: 'Support Desk',    icon: '✉', kind: 'source', tier: 'Demand', cost: 700,    monthlyCost: 300,   computeDraw: 3,   computeSupply: 0, dataRisk: 1, laborLoad: 3, color: '#8a7f6b', description: 'Inbound tickets from paying customers. Higher value work, real SLAs.' }),
  B({ id: 'doc_ingest',    name: 'Document Ingest', icon: '▤', kind: 'source', tier: 'Demand', cost: 900,    monthlyCost: 150,   computeDraw: 4,   computeSupply: 0, dataRisk: 2, laborLoad: 2, color: '#9e8a6b', description: 'Customer corpora. Unstructured charges $0.015/page; LlamaParse ~$0.00125.' }),
  ...
  B({ id: 'qdrant_local',  name: 'Qdrant (Self-Hosted)',icon:'◴',kind: 'factory', tier: 'Retrieval', cost: 350,   monthlyCost: 0,    computeDraw: 70,  computeSupply: 0, dataRisk: 0, color: '#a83246', description: 'Apache 2.0, written in Rust, ships as one Docker container. No per-query bill and no subprocessor to disclose — but you are the one who keeps it up at 2am.' }),
  B({ id: 'redis_cache',   name: 'Prompt Cache',    icon: '◷', kind: 'capacity', tier: 'Retrieval', cost: 250,   monthlyCost: 20,   computeDraw: 0,  computeSupply: 900, dataRisk: 0, powerKw: 0.3, color: '#dc382d', description: 'Cache reads bill at 10% of input across every major provider — 3.3% on DeepSeek. Repeat prefixes become free throughput.' }),

  // ======================================================================
  // AGENTS — orchestration, tools, evals
  // ======================================================================
  B({ id: 'human_ops',     name: 'Human Ops Desk',  icon: '☏', kind: 'factory', tier: 'Agents', cost: 1200, monthlyCost: 260, computeDraw: 8,   computeSupply: 0, dataRisk: 0, laborLoad: 4, opsRole: 'renewals', color: '#6b9e8a', description: 'Two people you hire to phone customers back. They re-sign a lapsed contract on their own — slowly, one at a time, at the same quarter-price fee — and they do it years before you can afford an agent that does it instantly. They are also the first payroll the footprint meter ever sees.' }),
  ...
  B({ id: 'support_agent',    name: 'Support Agent',        icon: '☎', kind: 'agent', tier: 'Agent Ops', agentRole: 'support',   agentDrift: 5,  agentStrength: 0.55, cost: 8000,  monthlyCost: 1400, computeDraw: 140, computeSupply: 0, dataRisk: 1, color: '#5b93a8', description: 'Customers leave the ones nobody looks after. Holds the accounts it watches against churn and re-signs them when their term runs out, paying out of your cash without asking. The only node in the addon whose whole job is preventing a bad outcome rather than causing a good one.' }),
  ...
  // ======================================================================
  // SUSTAINABILITY — the ESG addon. Nothing here sells an answer.
  //
  // Every node is overhead bought against a number that was already running:
  // the power, the water, the land, the people and the paperwork the rest of
  // the factory has been externalising since the first Gaming PC. The one
  // exception is Heat Recovery, which turns the largest liability in the addon
  // back into a revenue line.
  // ======================================================================
  B({ id: 'sustainability_officer', name: 'Sustainability Officer', icon: '⚘', kind: 'factory', tier: 'Sustainability', maxCount: 1, cost: 12000,  monthlyCost: 2400,  computeDraw: 10, computeSupply: 0, dataRisk: 0, provenanceRisk: -1, color: '#5f9e7a', description: 'Somebody has to sign the disclosure. Required before you can publish one at all, and required before anything else in this tab. A person, not a dashboard.' }),
  B({ id: 'renewable_ppa',   name: 'Renewable PPA',      icon: '⌁', kind: 'factory', tier: 'Sustainability', maxCount: 1, cost: 0,      monthlyCost: 26000, computeDraw: 2,  computeSupply: 0, dataRisk: 0, cleanFraction: 0.6, color: '#4ca86b', description: 'Contract 60% of your load to a wind or solar project. It cuts the carbon term and not one cent of the bill — a PPA is a price hedge, and annual matching is not the same as running clean at 3am.' }),
  B({ id: 'demand_response', name: 'Demand Response',    icon: '⎓', kind: 'factory', tier: 'Sustainability', maxCount: 1, cost: 18000,  monthlyCost: 900,   computeDraw: 4,  computeSupply: 0, dataRisk: 0, color: '#c9a13d', description: 'Shed load when the grid is tight and the utility pays you for it. The honest node in this tab: it saves real money and saves no carbon whatsoever.' }),
  B({ id: 'closed_loop',     name: 'Closed-Loop Retrofit',icon: '◍', kind: 'factory', tier: 'Sustainability', maxCount: 1, cost: 140000, monthlyCost: 4000,  computeDraw: 2,  computeSupply: 0, dataRisk: 0, color: '#4a8fa8', description: 'Rip out the evaporative towers and run sealed. Unlocks closed-loop cooling on every node you own. Water falls to almost nothing and the power bill goes back up — there is no third option.' }),
  B({ id: 'heat_recovery',   name: 'Heat Recovery Loop', icon: '♨', kind: 'factory', tier: 'Sustainability', cost: 26000,  monthlyCost: 1200,  computeDraw: 4,  computeSupply: 0, dataRisk: 0, color: '#e0704a', description: 'Nearly all the power a rack draws leaves as heat at about 30C. Catch it, lift it with a heat pump, and it becomes something a district network will buy. Needs real hardware on site to be worth anything.' }),
  B({ id: 'carbon_desk',     name: 'Carbon Credits Desk',icon: '❋', kind: 'factory', tier: 'Sustainability', maxCount: 1, cost: 4000,   monthlyCost: 1200,  computeDraw: 2,  computeSupply: 0, dataRisk: 0, provenanceRisk: 2, color: '#8aa85b', description: 'The cheapest lever in this tab by a wide margin, which is the first thing worth noticing about it. Buys tonnes off a registry you did not audit, and the auditor discounts them accordingly.' }),
  B({ id: 'annotation_coop', name: 'Annotation Co-op',   icon: '☗', kind: 'factory', tier: 'Sustainability', cost: 22000,  monthlyCost: 6000,  computeDraw: 8,  computeSupply: 0, dataRisk: -1, laborLoad: -6, color: '#6bb85b', description: 'Labelling done by people on a living wage with a contract and a psychologist on call. Moderators on one outsourced frontier-lab contract in Nairobi were reported at $1.32 to $2 an hour. This costs what that did not.' }),
  B({ id: 'trust_safety',    name: 'Trust & Safety Desk',icon: '⚖', kind: 'factory', tier: 'Sustainability', cost: 15000,  monthlyCost: 4500,  computeDraw: 20, computeSupply: 0, dataRisk: -2, slopRisk: -3, laborLoad: -4, color: '#5b93a8', description: 'Reviews what the pipeline is about to ship and what it is doing to the people who look at it. Cuts the Slop Index as well — the two problems were always the same problem.' }),
  B({ id: 'provenance_ledger',name:'Provenance Ledger',  icon: '⛓', kind: 'factory', tier: 'Sustainability', maxCount: 1, cost: 30000,  monthlyCost: 1800,  computeDraw: 30, computeSupply: 0, dataRisk: -2, provenanceRisk: -6, color: '#7a8fd8', description: 'C2PA manifests on what you ship and data cards on what you trained. Knowing where every input came from is unglamorous, expensive, and the only thing that survives an audit.' }),
  B({ id: 'esg_auditor',     name: 'ESG Audit',          icon: '⌑', kind: 'factory', tier: 'Sustainability', maxCount: 1, cost: 40000,  monthlyCost: 3200,  computeDraw: 20, computeSupply: 0, dataRisk: 0, provenanceRisk: -2, color: '#a8a05b', description: 'An assurance firm with its name on the number. Turns the ledger into a disclosure somebody else is liable for, which is the only reason a procurement officer believes it.' }),

  // ======================================================================
  // CAPACITY — throughput. Everything else throttles without it.
  // ======================================================================
  B({ id: 'free_tier',     name: 'Free Tier',       icon: '○', kind: 'capacity', vendorScoped: true, servesNodes: 1, maxCount: 1, tier: 'Capacity', cost: 0,     monthlyCost: 0,     computeDraw: 0, computeSupply: 150,      dataRisk: 0, powerKw: 0, color: '#5b7f5b', description: 'The free developer account every provider hands out when you sign up. Enough to prove the idea works and nothing more.' }),
  B({ id: 'api_tier1',     name: 'API Tier 1',      icon: '①', kind: 'capacity', vendorScoped: true, tier: 'Capacity', cost: 200,   monthlyCost: 0,     computeDraw: 0, computeSupply: 500,     dataRisk: 0, powerKw: 0.5, color: '#4a8f4a', description: 'The same account after $5 paid: 500k TPM, $100/mo spend cap. The real first wall you hit.' }),
  B({ id: 'api_tier3',     name: 'API Tier 3',      icon: '③', kind: 'capacity', vendorScoped: true, tier: 'Capacity', cost: 1800,  monthlyCost: 0,     computeDraw: 0, computeSupply: 5000,    dataRisk: 0, powerKw: 4, color: '#4a9f5a', description: 'The account after $100 paid. $1,000/mo cap. Tiers are earned by spending, not by asking.' }),
  B({ id: 'api_tier5',     name: 'API Tier 5',      icon: '⑤', kind: 'capacity', vendorScoped: true, tier: 'Capacity', cost: 14000, monthlyCost: 0,     computeDraw: 0, computeSupply: 40000,   dataRisk: 0, powerKw: 30, color: '#4aaf6a', description: 'The account after $1,000 paid. 40M TPM and a $200,000/mo cap. The ceiling of renting someone else’s compute.' }),
  B({ id: 'batch_lane',    name: 'Batch Lane',      icon: '◫', kind: 'capacity', vendorScoped: true, tier: 'Capacity', cost: 900,   monthlyCost: 0,     computeDraw: 0, computeSupply: 9000,    dataRisk: 0, powerKw: 5, color: '#3a8f7a', description: 'Batch APIs are 50% off at OpenAI, Anthropic, Google and Fireworks. Half price, but you wait.' }),
  B({ id: 'rented_h100',   name: 'Rented H100 Node',icon: '▣', kind: 'capacity', tier: 'Capacity', cost: 0,     monthlyCost: 15100, computeDraw: 0, computeSupply: 24000,   dataRisk: 0, powerKw: 10, landUse: 1, color: '#76b900', description: '8x H100 at RunPod ~$2.59/hr. Median across 40 providers is $3.32/hr; GCP charges $11.07. Unlocks self-hosting.' }),
  B({ id: 'rented_b200',   name: 'Rented B200 Pod', icon: '▩', kind: 'capacity', tier: 'Capacity', cost: 0,     monthlyCost: 39600, computeDraw: 0, computeSupply: 70000,   dataRisk: 0, powerKw: 14, landUse: 1, color: '#8fd400', description: '8x B200 at ~$6.79/hr. Spot cuts it 60-75% but interrupts; 3-year reserved is ~45% off and does not.' }),
  B({ id: 'colo_rack',     name: 'Colo Rack Bay',   icon: '⛁', kind: 'capacity', tier: 'Capacity', cost: 250000,monthlyCost: 42000, computeDraw: 0, computeSupply: 130000,  dataRisk: 0, powerKw: 120, landUse: 4, color: '#5a8f3a', description: 'Burns your own NVL72 racks. ~120kW each, liquid cooling mandatory — air alone cannot cool it.' }),
  B({ id: 'datacenter',    name: 'Own Datacenter',  icon: '▰', kind: 'capacity', tier: 'Capacity', cost: 380000000, monthlyCost: 7080000, computeDraw: 0, computeSupply: 2600000, dataRisk: 0, powerKw: 10000, landUse: 10, color: '#3a6f2a', description: '10MW. All-in capex runs $30-45M per MW, annualised TCO ~$8.5M/MW/yr. Servers are 60% of it; power only ~7%.' }),
  ...
  B({ id: 'data_curation', name: 'Data Curation',   icon: '≋', kind: 'factory', tier: 'Training', cost: 4000,    monthlyCost: 800,   computeDraw: 200,   computeSupply: 0, dataRisk: 3, laborLoad: 5, provenanceRisk: 4, color: '#6b7f6b', description: 'Common Crawl is free; licensed corpora are $10M-250M annual lump sums. Dedupe and filter or you train on slop.' }),
  ...
  // ======================================================================
  B({ id: 'poly_supply',   name: 'Polysilicon Supply',icon:'◆',kind: 'source',  tier: 'Silicon', cost: 60000,      monthlyCost: 9000,     computeDraw: 30,   computeSupply: 0, dataRisk: 0, powerKw: 40, landUse: 4, provenanceRisk: 3, color: '#9e8a6b', description: 'The cheapest input in the entire chain, and the start of every chip on earth.' }),
  B({ id: 'wafer_slicer',  name: 'Wafer Fab',       icon: '▬', kind: 'factory', tier: 'Silicon', cost: 400000,     monthlyCost: 60000,    computeDraw: 200,  computeSupply: 0, dataRisk: 0, powerKw: 120, landUse: 5, provenanceRisk: 2, color: '#d8b04a', description: 'Grow the ingot, slice, polish. 300mm blanks are ~$45 before anyone interesting touches them.' }),
  B({ id: 'euv_litho',     name: 'EUV Lithography', icon: '▭', kind: 'factory', tier: 'Silicon', cost: 380000000,  monthlyCost: 4000000,  computeDraw: 900,  computeSupply: 0, dataRisk: 0, powerKw: 1000, landUse: 6, provenanceRisk: 2, color: '#e0d13c', description: 'ASML EXE:5200B is ~$380M, weighs 150 tonnes and ships in 250 crates. No EUV, no sub-7nm. This is the chokepoint.' }),
  B({ id: 'die_test',      name: 'Die Test & Dicing',icon: '◫',kind: 'factory', tier: 'Silicon', cost: 900000,     monthlyCost: 120000,   computeDraw: 400,  computeSupply: 0, dataRisk: 0, powerKw: 200, landUse: 3, provenanceRisk: 2, color: '#e08a3c', description: 'TSMC N2 yields 70-80% on logic test chips. Yield loss here is why the last 20% of a node takes years.' }),
  B({ id: 'hbm_stacker',   name: 'HBM Stacking',    icon: '▥', kind: 'factory', tier: 'Silicon', cost: 2400000,    monthlyCost: 300000,   computeDraw: 500,  computeSupply: 0, dataRisk: 0, powerKw: 300, landUse: 4, provenanceRisk: 4, color: '#f26d3d', description: '12-Hi HBM4 runs ~$550 a stack and SK hynix took ~$560/device from NVIDIA. 2026 supply is sold out.' }),
  B({ id: 'cowos_pack',    name: 'CoWoS Packaging', icon: '▤', kind: 'factory', tier: 'Silicon', cost: 6000000,    monthlyCost: 700000,   computeDraw: 700,  computeSupply: 0, dataRisk: 0, powerKw: 400, landUse: 5, provenanceRisk: 4, color: '#ff6d3d', description: 'The real ceiling on GPU supply. Booking window 52-78 weeks; 85%+ of 2026-27 capacity is already locked.' }),
  B({ id: 'rack_integrator',name:'Rack Integration',icon: '⛁', kind: 'factory', tier: 'Silicon', cost: 3000000,    monthlyCost: 400000,   computeDraw: 600,  computeSupply: 0, dataRisk: 0, powerKw: 150, landUse: 3, provenanceRisk: 2, color: '#ffd24a', description: '72 accelerators, ~120kW, ~$3M a rack. NVIDIA captures ~90% of system value; integrators keep the scraps.' }),

  // ======================================================================
  // CONTRACTS — customers. This is the only place money comes from.
  // ======================================================================
  B({ id: 'consumer_app',  name: 'Consumer App',    icon: '◎', kind: 'contract', tier: 'Contracts', cost: 0,       monthlyCost: 20,   computeDraw: 4,  computeSupply: 0, dataRisk: 1, color: '#8a8f9e', description: 'Prosumer subscriptions. Free to stand up — a signup form is not a capital project — and then it bills every month you keep it. Pays little per answer, never does security review, churns constantly.' }),
  ...
  B({ id: 'parts_shop',  name: 'Parts Distributor',icon: '▦', kind: 'source',  tier: 'Home Lab', cost: 250,   monthlyCost: 0,   computeDraw: 2,  computeSupply: 0, dataRisk: 0, provenanceRisk: 2, color: '#8a94a8', description: 'CPU, memory, storage, a box to put them in. Two of those four have quadrupled since 2024 and two have not moved.' }),
  B({ id: 'gpu_market',  name: 'GPU Market',       icon: '◨', kind: 'source',  tier: 'Home Lab', cost: 300,   monthlyCost: 0,   computeDraw: 2,  computeSupply: 0, dataRisk: 0, provenanceRisk: 3, color: '#76b900', description: 'New, used, and whatever a data centre is decommissioning this quarter. VRAM per dollar has got worse every generation since 2020.' }),
  ...
  B({ id: 'byo_bay',     name: 'BYO Rack Bay',     icon: '▣', kind: 'capacity',tier: 'Home Lab', cost: 350,   monthlyCost: 0,  computeDraw: 0,  computeSupply: 0, dataRisk: 0, powerKw: 0.45, color: '#7fa860', description: 'A bare rig, a graphics card and a power supply. Cheapest throughput you can own — until the power supply decides otherwise.' }),

  B({ id: 'gaming_pc',   name: 'Gaming PC',        icon: '▤', kind: 'capacity',tier: 'Home Lab', cost: 1200,  monthlyCost: 0,  computeDraw: 0, computeSupply: 90,   dataRisk: 0, powerKw: 0.5, color: '#5b8f4a', priceElasticity: 0.25, description: 'One used 3090 and 24GB of VRAM. The cheapest way to run an 8B at speed, and 24GB is the wall every hobbyist hits.' }),
  B({ id: 'mac_mini',    name: 'Mac Mini M4 Pro',  icon: '▢', kind: 'capacity',tier: 'Home Lab', cost: 2199,  monthlyCost: 0,   computeDraw: 0, computeSupply: 70,   dataRisk: 0, powerKw: 0.15, color: '#a8a8b0', priceElasticity: 0.20, description: '64GB unified at 273 GB/s. Slow, silent, and it holds models a 5090 cannot.' }),
  B({ id: 'framework_pc',name: 'Framework Desktop',icon: '▥', kind: 'capacity',tier: 'Home Lab', cost: 1999,  monthlyCost: 0,  computeDraw: 0, computeSupply: 95,   dataRisk: 0, powerKw: 0.25, color: '#e07a3c', priceElasticity: 0.28, description: 'Strix Halo, 128GB unified with 96GB allocatable to the GPU, 256 GB/s. Best dollars per gigabyte on the shelf.' }),
  B({ id: 'dgx_spark',   name: 'DGX Spark',        icon: '◧', kind: 'capacity',tier: 'Home Lab', cost: 3999,  monthlyCost: 0,  computeDraw: 0, computeSupply: 130,  dataRisk: 0, powerKw: 0.24, color: '#76b900', priceElasticity: 0.30, description: 'About 1 PFLOP of FP4 against 273 GB/s of feed. Launched at $3,999 and was repriced to $4,699 in February 2026 on memory supply alone.' }),
  B({ id: 'mac_studio_512',name:'Mac Studio 512GB',icon: '◫', kind: 'capacity',tier: 'Home Lab', cost: 9499,  monthlyCost: 0,  computeDraw: 0, computeSupply: 420,  dataRisk: 0, powerKw: 0.27, color: '#c8c8d0', priceElasticity: 0.22, withdrawnAtIndex: 3.4, description: '512GB unified at 819 GB/s — the only machine here that runs a 671B model in 4-bit, off a wall socket. Apple withdrew this configuration in March 2026 rather than reprice it.' }),
  B({ id: 'mac_studio_256',name:'Mac Studio 256GB',icon: '◪', kind: 'capacity',tier: 'Home Lab', cost: 7499,  monthlyCost: 0,  computeDraw: 0, computeSupply: 380,  dataRisk: 0, powerKw: 0.27, color: '#b8b8c0', priceElasticity: 0.26, description: 'What is left after the withdrawal. The 96GB to 256GB upgrade went from $1,600 to $2,000 the same week.' }),
  B({ id: 'dgx_station', name: 'DGX Station GB300',icon: '▰', kind: 'capacity',tier: 'Home Lab', cost: 79000, monthlyCost: 0, computeDraw: 0, computeSupply: 3400, dataRisk: 0, powerKw: 1.6, color: '#4a8f2a', priceElasticity: 0.32, description: '784GB coherent memory — a datacenter node in a deskside box. The price is an estimate; NVIDIA has never published one.' }),
  B({ id: 'homelab_rack',name: 'Homelab Rack',     icon: '⛁', kind: 'capacity',tier: 'Home Lab', cost: 22000, monthlyCost: 0, computeDraw: 0, computeSupply: 6200, dataRisk: 0, powerKw: 1.8, landUse: 1, color: '#5a9f3a', priceElasticity: 0.20, description: 'Four ex-datacenter A100 40GB at about $5,000 each. Three years old, fully depreciated, and still the best dollars per kTPM in the game.' }),
  ...
  B({ id: 'text_mill',   name: 'Text Mill',        icon: '≡', kind: 'factory', tier: 'Slop', cost: 200,   monthlyCost: 0,   computeDraw: 90,  computeSupply: 0, dataRisk: 1, laborLoad: 2, slopRisk: 4, color: '#9e8fa8', description: 'Roughly 57% of sentences on the open web are already machine-translated. You are not starting a trend.' }),
  B({ id: 'image_gen',   name: 'Image Gen',        icon: '▩', kind: 'factory', tier: 'Slop', cost: 400,   monthlyCost: 0,   computeDraw: 220, computeSupply: 0, dataRisk: 2, laborLoad: 2, slopRisk: 5, color: '#c47fd0', description: 'About four cents an image at standard quality. Cheap enough that nobody stops to ask whether they wanted it.' }),
  ...
  B({ id: 'packager',    name: 'Content Packager', icon: '❑', kind: 'factory', tier: 'Slop', cost: 1200,  monthlyCost: 120, computeDraw: 60,  computeSupply: 0, dataRisk: 1, laborLoad: 3, slopRisk: 2, color: '#c98a3d', description: 'Text, images and video assembled into something a buyer takes delivery of. This is the node that makes the difference between a hobby and a business.' }),
  ...
  B({ id: 'nsfw_studio', name: 'NSFW Studio',      icon: '◉', kind: 'factory', tier: 'Slop', cost: 15000, monthlyCost: 900, computeDraw: 700, computeSupply: 0, dataRisk: 9, laborLoad: 7, slopRisk: 9, color: '#b03d5f', description: 'Nine points of Exposure, and the abliteration rig it needs adds six more. Fifteen is over the ceiling on Health/Finance and Federal, and level with Enterprise — read the ceilings before you wire this up.' }),
  ...
  B({ id: 'heat_offtake',name: 'District Heat Offtake',icon:'♨', kind: 'contract',tier: 'Contracts', addon: 'esg', cost: 9000, monthlyCost: 600, computeDraw: 4, computeSupply: 0, dataRisk: 0, color: '#e0704a', description: 'The municipal heat utility next door will take everything you can lift to 70C. Stockholm has been buying data centre heat for years and Meta gives Odense its waste heat for nothing. It pays little and it never churns.' }),
  ...
  B({ id: 'content_mill',name: 'Content Mill',     icon: '✍', kind: 'contract',tier: 'Contracts', cost: 900,   monthlyCost: 0, computeDraw: 4,  computeSupply: 0, dataRisk: 1, laborLoad: 6, slopRisk: 3, color: '#c98a5f', description: 'Over a thousand unreliable AI-generated news sites were being tracked by 2025. They pay per piece, on delivery, and they will not automate for you.' }),
  B({ id: 'pseo_platform',name:'Programmatic SEO', icon: '⌸', kind: 'contract',tier: 'Contracts', cost: 12000, monthlyCost: 0, computeDraw: 10, computeSupply: 0, dataRisk: 1, laborLoad: 4, slopRisk: 4, color: '#b8923d', description: 'Less per unit than clicking Generate yourself, and it runs while you are asleep. That is the entire argument for building a pipeline.' }),
  B({ id: 'adult_platform',name:'Adult Platform',  icon: '◐', kind: 'contract',tier: 'Contracts', cost: 40000, monthlyCost: 0, computeDraw: 12, computeSupply: 0, dataRisk: 2, laborLoad: 7, slopRisk: 6, color: '#a03d5f', description: 'Age-assurance regimes went live in the UK and were upheld for Texas at the US Supreme Court in 2025. Payment processors moved on storefronts the same summer.' }),
];
